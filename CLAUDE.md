...
- **setup.py** — interactive setup wizard: generates config.yaml, .env, Caddyfile, crontab; tests API connections; initializes DB
...
- `Dockerfile` — multi-stage build (builder + runtime), includes supercronic for container cron
- `docker-compose.yml` — three services: `web` (uvicorn), `cron` (supercronic), `caddy` (auto-HTTPS reverse proxy)
- `config.yaml.example` — generic config template used by setup wizard
- `Caddyfile` — generated by setup wizard, or manually edited for custom domain

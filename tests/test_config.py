...
class TestConfigLoading:
    ...
    def test_missing_topic_raises(self):
        path = _write_config({"scoring": {"alpha": 0.5}})
        with pytest.raises(ValueError, match="topic"):
            load_config(path)

    def test_missing_name_raises(self):
        path = _write_config({"topic": {"primary_keywords": ["a"]}})
    ...
    def test_custom_scoring(self):
        ...
        assert config.scoring.quality.w_venue == 0.5

    def test_llm_defaults_disabled(self):
        path = _write_config({
            "topic": {
                "name": "Test",
                "primary_keywords": ["test"],
    ...
    def test_llm_cost_control(self):
        path = _write_config({
            "topic": {
                "name": "Test",
                "primary_keywords": ["test"],
